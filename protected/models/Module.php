...
class Module extends CActiveRecord
{
    ...
    protected static function getModulesList()
    {
        ...
        $listModulesJson = file_get_contents($updateServer.'/update/getModules');
    ...
    protected static function downloadModuleArchive($module)
    {
        $updateServer = trim(Yii::app()->params['updateServer'],'/');
        $moduleArchive = file_get_contents($updateServer.'/update/getModuleArchiveLink/name/'.strtolower($module));
        if ($moduleArchive !== false && !empty($moduleArchive)) {
            $tempName = uniqid().'.zip';
            if (file_put_contents(YiiBase::getPathOfAlias('webroot.updates.archives').DIRECTORY_SEPARATOR.$tempName, file_get_contents($moduleArchive)) !== false)
                return $tempName;
        }
        return false;
    ...
    public static function install($module, $archiveName=null)
    {
        $archiveModuleName = false;
        if ($archiveName !== null)
            $archiveModuleName = $archiveName;
        else
            $archiveModuleName = self::downloadModuleArchive($module);
    ...
    {
        if (!$module || !file_exists(realpath(Yii::app()->basePath . DIRECTORY_SEPARATOR .'modules' .DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.'controllers')))
            return array();
        ...
        foreach($files as $file){
            ...
            if( ($pos = strpos($filename, 'Controller')) > 0){
                $controllers[substr($filename, 0, $pos)] = substr($filename, 0, $pos);
            }
        }
        return $controllers;
    }
    ...
    public static function listMethods($module, $controller)
    {
        if (!$module || !$controller)
            return array();
        Yii::import('application.modules.'.strtolower($module).'.controllers.'.$controller.'Controller',true);
        $reflection = new ReflectionClass($controller.'Controller');
        $methods = $reflection->getMethods();
        $resultArray = array();
        foreach($methods as $method) {
            if (preg_match('/^action+\w{2,}/',$method->name)) {
                $resultArray[substr($method->name, strlen('action'))] = substr($method->name, strlen('action'));
            }
        }
        return $resultArray;
    }
