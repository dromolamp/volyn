    ...
    protected function renderButton($id,$button,$row,$data)
    {
        ...
        if (!$this->editableOrdering && $value != 1)
            echo CHtml::link(
                ...
                Yii::app()->controller->createUrl("ordering", array("id"=>$data->primaryKey, 'action'=>'up', 'attribute'=>$this->name)),
                $this->buttonOptions
            );
        if ($this->editableOrdering) {
            echo CHtml::beginForm(
                Yii::app()->controller->createUrl('ordering', array("id"=>$data->primaryKey, 'action'=>'setValue', 'attribute'=>$this->name)),
                'post',
                array(
                    'class'=>'setOrdering'
                )
        ...
        if (!$this->editableOrdering && $value != $totalCountItems)
            echo CHtml::link(
                ...
                Yii::app()->controller->createUrl("ordering", array("id"=>$data->primaryKey, 'action'=>'down', 'attribute'=>$this->name)),
                $this->buttonOptions
            );
    }
