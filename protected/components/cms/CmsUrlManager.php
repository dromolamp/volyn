...
class CmsUrlManager extends CUrlManager
{
    ...
    public function parseModulesRoute()
    {
        ...
        {
            if (is_file(YiiBase::getPathOfAlias('application.modules.'.$key).DIRECTORY_SEPARATOR."route.php"))
            {
                $route = require(YiiBase::getPathOfAlias('application.modules.'.$key).DIRECTORY_SEPARATOR."route.php");
                $order[] = $route['order'];
                $rules[] = $route['rules'];
            }
        }
