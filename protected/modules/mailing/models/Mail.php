...
class Mail extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('id, name, text, pub_date, status', 'safe', 'on'=>'search'),
		);
	}
    ...
    public function sendEmail($mail_id)
    {
        ...
        $mail = self::model()->findByPk($mail_id);
        $usersEmail = array();
        foreach ($users as $user)
            if (!empty($user->email))
                $usersEmail[$user->email] = $user->username;
        return SendMail::send($usersEmail, $mail->name, $mail->text);
    }
}